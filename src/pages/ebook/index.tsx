...
const EBOOK_COVER_FALLBACK = "/images/ebook-placeholder.jpg";
  ...
  function mapDtoToViewModel(dto: EbooksGetDto): EbookViewModel {
    ...
    const content = parseEbookContent(
      dto.tableOfContents,
      author
    );
  ...
  export default function EbookPage() {
    ...
    const ebook =  ebooks.find((item) => item.id === activeEbookId) ??  ebooks[0] ??  null;
    const activeEbookIndex = ebook ? ebooks.findIndex((item) => item.id === ebook.id) : -1;

    function switchToEbook(
      nextId: number,
    ...
    function showNextEbook() {
      ...
      const nextIndex =
        activeEbookIndex === ebooks.length - 1
          ? 0
          : activeEbookIndex + 1;
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  switchToEbook(
                                    item.id,
                                    nextIndex < activeEbookIndex ? "left" : "right"
                                  );
                                }}                                className={[
                                  "h-2.5 rounded-full transition-all",
                                  selected
                                    ? "w-7 bg-[rgb(var(--primary))]"
                                    : "w-2.5 bg-[rgba(var(--primary),0.25)] hover:bg-[rgba(var(--primary),0.5)]",
                                ].join(" ")}
